...
const Navbar = React.memo(({ logged, val, setVal }) => {
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-start", width: "25%" }}>
          <Link style={{ textDecoration: "none", color: "inherit" }} to="/">
            <img className="navLogo" width={100} height={50} src={useMemo(() => Icon, [])} alt="Logo" />
          </Link>
        </div>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", width: "80%" }}>
          <div style={{ display: "flex", alignItems: "center" }}>
            <Autocomplete
              ...
              id="combo-box-demo"
              options={titles}
              renderInput={(params) => <TextField onSelect={(e) => { setSearchQuery(e.target.value) }} onChange={(e) => { setSearchQuery(e.target.value) }} {...params} label="" />}
            />
            <input onClick={handleSearch} style={{ width: 100 }} className="InputBtn" type="submit" value="Search" />
          </div>
          <div onClick={handleClickOpen}>
            <ul style={{ width: 190, cursor: "pointer" }} className="area" onClick={() => { setOpen(true) }}>
              <GpsNotFixedIcon />
              <span className="navLocation signin" style={{ fontSize: `${!currentUser ? 10 : 15}`, fontWeight: "600" }}>{location === "Newfoundland and Labrador" ? "Newfoundland" : location}</span>
            </ul>
          </div>
        </div>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", width: "25%" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
            ...
            {currentUser &&
              <PopupState className="demo-popup-menu" variant="popper" popupId="demo-popup-menu">
                {(popupState) => (
                  <React.Fragment>
      ...
      <div className="container navElements">
        <div className="row">
          <div
            ...
            className="col-lg-12 navOptions"
          >
